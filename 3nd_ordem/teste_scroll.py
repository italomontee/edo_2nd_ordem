...
scrollbar3 = tk.Scrollbar(tab3, orient="vertical", command=canvas.yview)
scrollbar3.grid(row=0, column=1, sticky="ns")  # Use grid
# Configure the Canvas to work with the scrollbar
canvas.config(yscrollcommand=scrollbar3.set)
canvas.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))

# Valores pedidos ao usuario
derivative_label3 = tk.Label(scrollable_frame, text="Digite a derivada de y (y'): ")
derivative_label3.pack(pady=(10, 0))

# ... Other entries, labels, and buttons ...

# Create a Text widget to display the results
result_text3 = tk.Text(scrollable_frame, wrap=tk.WORD, width=25, height=7.5)
result_text3.pack(padx=10, pady=10)

# ...

# Update the scrollable region when the content changes
def update_scroll_region(event):
    canvas.configure(scrollregion=canvas.bbox("all"))

result_text3.bind("<Configure>", update_scroll_region)
